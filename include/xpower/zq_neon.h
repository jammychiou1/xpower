...
namespace xpower::barret {
  ...
  inline static int16x8_t gen_esti(int16x8_t in, int16x8_t bar) {
    return vqrdmulhq_laneq_s16_asm<BAR_LANE>(in, bar);
  }

  template <int MOD_LANE>
  inline static int16x8_t use_esti(int16x8_t lhalf, int16x8_t esti, int16x8_t mod) {
    return vmlsq_laneq_s16_asm<MOD_LANE>(lhalf, esti, mod);
  }

  template <int COEF_LANE, int BAR_LANE, int MOD_LANE>
  inline static int16x8_t multiply(int16x8_t in, int16x8_t coef, int16x8_t bar, int16x8_t mod) {
  ...
  inline static int16x8_t shift_left(int16x8_t in, int16x8_t bar, int16x8_t mod) {
    int16x8_t esti = gen_esti<BAR_LANE>(in, bar);
  ...
  inline static int16x8_t reduce(int16x8_t in, int16x8_t _1_bar, int16x8_t mod) {
    int16x8_t esti = gen_esti<_1_BAR_LANE>(in, _1_bar);
    return use_esti<MOD_LANE>(in, esti, mod);
  }

  inline static int16x8_t gen_esti_crude_redc(int16x8_t in) {
  ...
  template <int MOD_LANE>
  inline static int16x8_t crude_redc(int16x8_t in, int16x8_t mod) {
...
namespace xpower::lhalf {
  ...
  inline static int16x8_t multiply(int16x8_t in, int16x8_t coef) {
    return vmulq_laneq_s16_asm<COEF_LANE>(in, coef);
  }

  template <int COEF_LANE>
  inline static int16x8_t mul_add(int16x8_t acc, int16x8_t in, int16x8_t coef) {
    return vmlaq_laneq_s16_asm<COEF_LANE>(acc, in, coef);
  }

  template <int COEF_LANE>
  inline static int16x8_t mul_sub(int16x8_t acc, int16x8_t in, int16x8_t coef) {
