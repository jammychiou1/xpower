...
namespace xpower::lowmul {
  void lowmul(const int16_t in1_low[81], const int16_t in2_low[81], int16_t out_full[1528]) {
    ...
    low_lay1::bwd_insert(out_arr, out_full);

    int64_t tmp = out_full[80] + 170 * (int64_t(in1_low[0]) * in2_low[80] + int64_t(in1_low[80]) * in2_low[0]);
    int64_t esti = (tmp * 935519 + (int64_t(1) << 31)) >> 32;
    out_full[80] = tmp - esti * shared::q;
